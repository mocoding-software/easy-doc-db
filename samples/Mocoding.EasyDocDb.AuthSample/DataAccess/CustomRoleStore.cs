...
namespace Mocoding.EasyDocDb.AuthSample.DataAccess
{
    public class CustomRoleStore : IRoleStore<IdentityRole<string>>
    {
        ...
        {
            var currentRole = _roles.All().FirstOrDefault(i => i.Data.Id == role.Id);
            await currentRole.SyncUpdate(d => d.NormalizedName = role.NormalizedName);
            await currentRole.SyncUpdate(d => d.Name = role.Name);
            await currentRole.SyncUpdate(d => d.Id = role.Id);
            await currentRole.SyncUpdate(d => d.ConcurrencyStamp = role.ConcurrencyStamp);
            return IdentityResult.Success;
        }
    }
}
